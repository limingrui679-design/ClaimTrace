...
|---|---|
| `npm run ci` | Passed |
| Unit and integration tests | 124/124 passed |
...
| Source-refresh consistency, recovery, and failure-path tests | 8/8 passed |
...
Normal `npm run cases:generate` execution is offline and deterministic. `npm run cases:refresh-sources` is a separate networked operation because a refresh can alter source content and downstream results. It validates both responses with the declared source-specific cleaner before replacing either pinned response, updates the retrieval timestamp, and leaves the selected case unchanged if either download fails, is empty, or fails source-schema and cleaning-parameter validation. It preflights all three targets and persists a transaction manifest. On the next invocation, an interrupted uncommitted replacement is restored before the case configuration is read, while committed work with unfinished cleanup is verified and cleaned; cleanup failures are surfaced to the caller.
