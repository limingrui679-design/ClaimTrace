...
`previousBundleHash` can link one bundle root to another. `verifyAuditBundleChain()` checks every supplied bundle independently, rejects missing/incorrect predecessors, duplicate or malformed roots, project/key discontinuity, invalid genesis links, and structurally malformed entries without throwing. A single package cannot prove an absent predecessor existed, and the chain is not externally anchored.
